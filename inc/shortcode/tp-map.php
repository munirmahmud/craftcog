...
class TP_Maps_Widget extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'CraftCog Map', 'craftcog' );
    }
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			array(
				'label' => esc_html__( 'Map Settings', 'craftcog' ),
			)
		);

		$default_address = esc_html__( '172 Antonovycha St. Kyiv 03150, Ukraine', 'craftcog' );

		$this->add_control(
			'map_center',
			array(
				'label'       => esc_html__( 'Map Center', 'craftcog' ),
		...
		$this->add_control(
			'zoom',
			array(
				'label'      => esc_html__( 'Initial Zoom', 'craftcog' ),
		...
		$this->add_control(
			'scrollwheel',
			array(
				'label'   => esc_html__( 'Scrollwheel Zoom', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'false',
				'options' => array(
					'true'  => esc_html__( 'Enabled', 'craftcog' ),
					'false' => esc_html__( 'Disabled', 'craftcog' ),
				),
			)
		);
		...
		$this->add_control(
			'zoom_controls',
			array(
				'label'   => esc_html__( 'Zoom Controls', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'craftcog' ),
					'false' => esc_html__( 'Hide', 'craftcog' ),
				),
			)
		);
		...
		$this->add_control(
			'street_view',
			array(
				'label'   => esc_html__( 'Street View Controls', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'craftcog' ),
		...
		$this->add_control(
			'map_type',
			array(
				'label'   => esc_html__( 'Map Type Controls (Map/Satellite)', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Show', 'craftcog' ),
		...
		$this->add_control(
			'drggable',
			array(
				'label'   => esc_html__( 'Is Map Draggable?', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'true',
				'options' => array(
					'true'  => esc_html__( 'Yes', 'craftcog' ),
					'false' => esc_html__( 'No', 'craftcog' ),
				),
			)
		);
		...
		$this->start_controls_section(
			'section_map_style',
			array(
				'label' => esc_html__( 'Map Style', 'craftcog' ),
			)
		);
		...
		$this->add_control(
			'map_height',
			array(
				'label'       => esc_html__( 'Map Height', 'craftcog' ),
				...
				'selectors' => array(
					'{{WRAPPER}} .craftcog-maps' => 'height: {{VALUE}}px',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'        => Controls_Manager::SELECT,
				'default'     => 'default',
				'options'     => $this->tp_get_map_style(),
				'label_block' => true,
				'description' => esc_html__( 'You can add own map styles within your theme. Add file with styles array in .json format into craftcog/google-map-styles/ folder in your theme. File must be minified', 'craftcog' )
			)
		);
		...
		$this->start_controls_section(
			'section_map_pins',
			array(
				'label' => esc_html__( 'Pins', 'craftcog' ),
			)
		);
		...
		$repeater->add_control(
			'pin_address',
			array(
				'label'       => esc_html__( 'Pin Address', 'craftcog' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => $default_address,
				'label_block' => true,
			)
		...
		$repeater->add_control(
			'pin_desc',
			array(
				'label'   => esc_html__( 'Pin Description', 'craftcog' ),
		...
		$repeater->add_control(
			'pin_image',
			array(
				'label'   => esc_html__( 'Pin Icon', 'craftcog' ),
		...
		$repeater->add_control(
			'pin_state',
			array(
				'label'   => esc_html__( 'Initial State', 'craftcog' ),
				'type'    => Controls_Manager::SELECT,
				'default' => 'visible',
				'options' => array(
					'visible' => esc_html__( 'Vsible', 'craftcog' ),
					'hidden'  => esc_html__( 'Hidden', 'craftcog' ),
				),
			)
		);
	...
	public function tp_get_map_style() {

		$style = array(

			'default' => esc_html__( 'Default', 'craftcog' ),
	...
	public function get_location_coord( $location ) {
		...
		$api_key = fw_get_db_customizer_option('tp_google_api');

		// Do nothing if api key is not provided
		if ( ! $api_key ) {
			return;
		}
	...
	protected function render() {
		...
		printf(
			'<div class="craftcog-maps" %1$s %2$s></div>',
