...
class FW_Option_Type_FwMultiInline extends FW_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		return fw_render_view( GEOBIN_INC . '/includes/option-types/fw-multi-inline/view.php', array(
			'id'	 => $id,
			'option' => $option,
			'data'	 => $data
		) );
	...
	protected function _get_defaults() {
		...
		return array(
			...
			'fw_multi_options'	 => array(
				'firstoption' => array(
					'type'		 => 'select',
					'title'		 => esc_html__( 'Title', 'craftcog' ),
					'choices'	 => array(
						'select1'	 => esc_html__( 'select1', 'craftcog' ),
						'select2'	 => esc_html__( 'select2', 'craftcog' ),
						'select3'	 => esc_html__( 'select3', 'craftcog' )
					),
				)
			),
			'groupname'			 => ''
