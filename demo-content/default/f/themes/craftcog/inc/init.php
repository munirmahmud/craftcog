...
class CraftCog_Theme_Includes {
	...
	public static function init() {
        ...
        if ( class_exists( 'TpCustomPost\TP_CustomPost' ) ) {

            /**
             *
             * Custom Posts And Taxonomies
             * new TpCustomPost\TP_CustomPost( 'Text Domain' )
             * $tab->tp_post_init( 'URL', 'Singular', 'Plural', array( 'menu_icon' => 'dashicons-wp', 'supports' => array( 'title', 'editor','thumbnail','excerpt' ), 'rewrite'=> array('slug'=>'plural_dahes') ) );
             * $tab_tax = new TpCustomPost\TP_Taxonomies( 'craftcog' );
             * $tab_tax->tp_post_init( 'service_cat', 'Service Category', 'Categories', 'service' );
             */
            $tab = new TpCustomPost\TP_CustomPost( 'craftcog' );
            $tab->tp_post_init( 'service', 'Service', 'Services', array( 'menu_icon'	 => 'dashicons-exerpt-view',
                'supports'	 => array( 'title', 'editor', 'thumbnail', 'excerpt' ),
                'rewrite'	 => array( 'slug' => 'service' ) ) );
            $tab->tp_post_init( 'projects', 'Project', 'Projects', array( 'menu_icon'	 => 'dashicons-exerpt-view',
                'supports'	 => array( 'title', 'editor', 'thumbnail', 'excerpt' ),
            ...
            /**
             *
             * Portfolio Custom Post And Taxonomies
             *
             */

            $portfolio_tax = new  TpCustomPost\TP_Taxonomies('craftcog');
            $portfolio_tax->tp_post_init('portfolio_cat', 'Category', 'Categories', 'projects');
        }
		 ...
		 */ {
			self::include_child_first( '/nav-walker.php' );
			self::include_child_first( '/helpers.php' );
			self::include_child_first( '/hooks.php' );
			self::include_child_first( '/includes/inline-styles.php' );
			...
			self::include_child_first( '/includes/tp-image-crop-class.php' );
	...
	public static function _action_enqueue_scripts() {
		self::include_child_first( '/theme-files.php' );
	}
	...
	public static function _action_widgets_init() { {
		...
		foreach ( $paths as $path ) {
			...
			foreach ( $dirs as $dir ) {
				...
				register_widget( 'CraftCog_' . self::dirname_to_classname( $dirname ) );
			}
		}
	}
